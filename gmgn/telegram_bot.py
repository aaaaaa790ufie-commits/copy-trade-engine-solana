...
DB=os.getenv("SENTINEL_DB","sentinel.db")
...
def snapshot(c):
    ...
    p=c.execute("SELECT token_mint,chain,stake_sol,entry_price,peak_price,opened_at FROM paper_positions WHERE status='open'").fetchall()
    return a,p
...
def text(c, command):
    ...
    if command=="/status":
        ...
        for x in p: out.append(f"{x[1]} {x[0][:8]}... | stake {x[2]:.4f} | entry {x[3]:.8g} | peak {x[4]:.8g}")
        return "\n".join(out)
    if command=="/trades":
        ...
        return "\n".join(f"{time.strftime('%Y-%m-%d %H:%M:%S UTC',time.gmtime(r[5]))} {r[2]} {r[0]} {r[1][:8]}... {r[3]*100:+.2f}% {r[4]:+.5f} SOL | {r[5]}" for r in rs) or "Сделок пока нет"
    if command=="/wallets":
...
def main():
    ...
    while True:
        try:
            data=api("getUpdates",{"timeout":25,"offset":offset}).get("result",[])
            for u in data:
